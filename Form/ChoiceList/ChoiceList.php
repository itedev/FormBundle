...
abstract class ChoiceList implements ChoiceListInterface
{
    ...
    protected function getNewValuesFromData($data)
    {
        ...
        $keys = range($maxIndex + 1, $maxIndex + count($newValues));
     ...
     * @param array $labels
     */
    protected function addNewChoices($choices, array $labels)
    {
        $this->addChoices(
            $this->preferredViews,
            $this->remainingViews,
     ...
     * @return void
     */
    abstract protected function addNewValues(array $values);
